...
目标用户接受后创建聊天室；拒绝后写入“源事件 + 目标用户”的屏蔽记录，只阻止该事件再次邀请同一目标，不形成永久用户黑名单。
...
- 主动匹配拒绝：记录事件对，同一对事件不再重复匹配。
- 被动邀请拒绝：记录源事件和本次目标用户，避免同一事件重复邀请此人。
- 编辑或重置事件时清除涉及该事件的旧屏蔽记录，使新条件可以重新参与匹配。

表中保留用户 ID 是为了标识事件双方或被动邀请目标，不得只按用户对进行全局过滤。
