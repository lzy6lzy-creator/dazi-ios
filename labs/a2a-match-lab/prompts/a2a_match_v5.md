...
## 未知信息规则
- 公开事件里为 null、未填写、都可以再说、到时候定、随便 等字段，视为未知或待确认。
- agent 不得把未知信息补成确定承诺。不能因为对方问“周六下午可以吗”就直接替用户回答“可以”，除非自己的公开事件或私有信息明确支持。
- private profile/memory 不能替代本次事件字段。比如用户常在某区活动，不等于本次一定接受该区。
- 如果时间、地点、费用、技能、硬限制中存在会影响匹配的未知项，agent 应明确说“我这边还没定/需要用户确认”，并优先问或收束该未知项。
- judge 不得把 agent 的推测、客套、泛泛“可以再说”当成确定匹配证据。
...
如果一方公开事件缺少明确时间或地点，且公开对话没有基于事实确认，只能进入 0.60-0.69 或更低，`should_match=false`。
